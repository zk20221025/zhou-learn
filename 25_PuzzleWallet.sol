...
//事实上，现在为DeFi操作付费是不可能的。 一群朋友发现了如何通过将多个交易批处理在一个交易中来略微降低执行多个交易的成本，因此他们为此开发了一个智能合约。 
...
//他们一点也不知道，他们的午餐钱正处于危险之中…… 您需要劫持这个钱包才能成为代理的管理员。 
 
//可能有帮助的事情:  
//理解委托调用如何工作以及msg如何工作。发件人和msg。Value在执行一个时的行为。 了解代理模式及其处理存储变量的方式。